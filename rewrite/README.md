...
│      ├── TenantBackendFactory.java  - Creates correct client        │
│      ├── jdbc/JdbcBackendClient.java - Database via HikariCP        │
...
### Single-Tenant Mode (Finvi)

One instance per customer. Configuration loaded from files.
...
| `infra/backend/jdbc/JdbcBackendClient.java` | Database connection (IRIS, SQL Server, etc.) |
...
| `backendType()` | required | `JDBC` or `REST` |
| `appName()` | auto | Application name for logs/Swagger |
| `enableJobStream()` | `true` | Enable job streaming from Gate |
...
To customize behavior for a specific tenant, extend a service class:

```java
public class CustomTransferService extends TransferService {
    @Override
    public TransferResponse executeTransfer(TransferRequest req) {
        // Custom pre-processing
        if (req.targetQueue().equals("restricted")) {
...
Then provide it to the builder:

```java
SatiApp.builder()
    .config(loadConfig())
    .backendType(BackendType.JDBC)
    .transferService(new CustomTransferService())
    .start(8080);
```
