...
public class TenantLogger {
  ...
  public TenantLogger(Logger logger) {
    this.logger = logger;
    this.locationAwareLogger =
        (logger instanceof LocationAwareLogger) ? (LocationAwareLogger) logger : null;
    this.tenant = null;
  }
  ...
  private void log(Level level, String format, Object arg) {
    if (locationAwareLogger != null) {
      locationAwareLogger.log(
          null, FQCN, toLocationAwareLevel(level), format, new Object[] {arg}, null);
    } else {
      switch (level) {
        case TRACE:
  ...
  private void log(Level level, String format, Object arg1, Object arg2) {
    if (locationAwareLogger != null) {
      locationAwareLogger.log(
          null, FQCN, toLocationAwareLevel(level), format, new Object[] {arg1, arg2}, null);
    } else {
      switch (level) {
        case TRACE:
